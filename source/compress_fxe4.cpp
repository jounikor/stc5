...
CompressFXE4::CompressFXE4( int nblks )
	throw (std::bad_alloc, std::invalid_argument) :
	LZ2min(FXE4_SLIDING_WINDOW,FXE4_BLOCK_SIZE,nblks) {

	_prehf      = new EncHuf(FXE4_PRE_SYMBOLS,7);
	_litmatchhf = new EncHuf(FXE4_NUM_LITMATCH,FXE4_MAX_CODELEN);
	_offhghhf   = new EncHuf(FXE4_NUM_OFFHGH,FXE4_MAX_CODELEN);
	_offlowhf   = new EncHuf(FXE4_NUM_OFFLOW,FXE4_MAX_CODELEN);
...
void CompressFXE4::init( const Params* p ) {
	...
	memset(_preCodes,0,FXE4_PRE_SYMBOLS);
	memset(_blockCodes,0,FXE4_NUM_SYMBOLS);
	memset(_deltaCodes,0,FXE4_NUM_SYMBOLS);
...
long CompressFXE4::compressBlock( const Params* info, bool& isRaw) {
...
//	unsigned char* e = lzp->buf + lzp->bufSize;
	...
	long oldOffset[FXE4_NUMPMR];
    long oldLength[FXE4_NUMPMR];
    long oldOffsetLong;
	...
	for (i = 0; i < lzp->c->getCount(); i++) {
		...
		if (len == FXEP_NO_MATCH) {
            ...
            for (j = 0; j < FXE4_NUMPMR; j++) {
                if (oldOffset[j] == off && oldLength[j] == len) {
                    sym = FXE4_PMR0 + j;
                    _litmatchhf->addCnt(sym);
                }
            }
            if (sym < 0) {
				sym = FXE4_HUFFBASE + len - 2;
				assert(sym < FXE4_NUM_LITMATCH);
				_litmatchhf->addCnt(sym);
				...
				for (j = 1; j < FXE4_NUMPMR; j++) {
                    oldOffset[j-1] = oldOffset[j];
                    oldLength[j-1] = oldLength[j];
                }
                oldOffset[j-1] = off;
				oldLength[j-1] = len;
			}
	...
	memcpy(_blockCodes,_litmatchhf->outputCodeLengthsBytes(),FXE4_NUM_LITMATCH);
	memcpy(_blockCodes+FXE4_NUM_LITMATCH,_offhghhf->outputCodeLengthsBytes(),FXE4_NUM_OFFHGH);
	memcpy(_blockCodes+FXE4_NUM_LITMATCH+FXE4_NUM_OFFHGH,_offlowhf->outputCodeLengthsBytes(),FXE4_NUM_OFFLOW);
	...
	memcpy(_preCodes,_prehf->outputCodeLengthsBytes(),FXE4_PRE_SYMBOLS);


	// output pre huffman tree.. 3 bits for each leaf..

	for (i = 0; i < FXE4_PRE_SYMBOLS; i++) {
	...
	for (i = j = 0; i < FXE4_NUM_SYMBOLS; i++) {
	...
	for (i = 0; i < lzp->c->getCount(); i++) {
		...
		if (len == FXEP_NO_MATCH) {
			...
			//assert(sym >= FXE4_HUFFBASE);
			assert(sym >= FXE4_PMR0);
			assert(sym < FXE4_NUM_LITMATCH);
			assert(_litmatchhf->getLength(sym) > 0);

			_putBits(p,_litmatchhf->getCode(sym),_litmatchhf->getLength(sym));

			if (sym >= FXE4_HUFFBASE) {
				if (off < 128) {
	...
	_putBits(p,_litmatchhf->getCode(FXE4_EOB),_litmatchhf->getLength(FXE4_EOB));
